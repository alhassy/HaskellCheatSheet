...
*Tuples* ~(α₁, …, αₙ)~ are types with values written ~(x₁, …, xₙ)~ {{{newline}}} where
...
[[https://en.wikipedia.org/wiki/Conjugacy_class][*Duality*]]: Let ~∂f = reverse . f . reverse~, then ~init = ∂ tail~ and {{{newline}}}
...
+ Look at the two cases of a function and move them to the two
  ...
  # - E.g., ~dropWhile p = foldr (λ x ys → if (p x) then ys else (x:ys)) []~.
# #
...
{{{newline}}} (Program statements, sequence “;”, the empty statement) ---this
...
*Naturality Theorems:* If ~p ∷ f a → g a~ for some /functors/ ~f~ and ~g~, {{{newline}}}
...
fmap is applied.
# - This is the naturality theorem for ~p~ where ~g a = ε~ is the constantly-~ε~ functor.
...
- The homomorphism law now becomes: ~pure . f = fmap f . pure~.
  # i.e., ~λ x → pure (f x) = λ x → fmap f (pure x)~.
...
Given ~f ∷ α₁ → ⋯ → αₙ → β~ and ~xᵢ ∷ f αᵢ~, we obtain an ~(f β)~-value. {{{newline}}}
...
:Table_with_pure:
| Functor     | ~f α~                  | ~pure a~           | ~f <*> x~                                |
|-------------+----------------------+------------------+----------------------------------------|
| Identity    | ~α~                    | ~a~                | ~f x~                                    |
| Constant    | ~ε~                    | ~mempty~           | ~f <> x~                                 |
| List        | ~[α]~                  | ~[a]~              | =fs <*> xs = [f x ∣ f <- fs, x <- xs]=   |
| Either      | ~Either ε α~           | ~Right a~      | ~right (λ f → right f ea) ef~            |
|-------------+----------------------+------------------+----------------------------------------|
| Composition | ~f₁ (f₂ α)~            | ~pure (pure a)~    | ~f <*> x = (<*>) <$> f <*> x~            |
| Product     | ~(f₁ α, f₂ α)~         | ~(pure a, pure a)~ | ~(f, g) <*> (x, y) = (f <*> x, g <*> y)~ |
| Sum         | ~Either (f₁ α) (f₂ α)~ |                  | ~f <$> ea = f +++ f~                       |
|-------------+----------------------+------------------+----------------------------------------|
| Writer      | ~(ε, α)~               | ~(mempty, a)~      | ~(a , f) <*> (b, x) = (a <> b, f x)~     |
| Reader      | ~ε → α~                | ~λ _ → a~          | ~f <*> g = λ x → f x (g x)~  --i.e., ~S~   |
:End:
...
we define {{{newline}}} ~do {x ← p; y ← qx; pure e} ≈ join $ ~(λ x -> (λ y → e) <$> qx) <$> p~.
...
#+BEGIN_SRC haskell
{- Proof:
   [base] do {do {D; r}} = do {D; r} ✓
   [ind]  do {x ← p; C′; do {D; r}}  -- case: x ∉ C
        = q <*> p where q x = do {C′; do {D; r}}
        = q <*> p where q x = do {C′; D; r}
        = do {x ← p; C′; D; r} ✓
   [ind]  do {x ← p; C′; do {D; r}}  -- case: x may be in C
        = join $ map q p where q x = do {C′; do {D; r}}
        = join $ map q p where q x = do {C′; D; r}
        = do {x ← p; C′; D; r} ✓
-}
#+END_SRC
:End:
...
# Cool.
# Notice that ~fmap f p = do { x ← p; pure (f x) } = join $ fmap (pure . f) p~
...
do {x ← p; do {C; r}} = do {x ← p; C; r} --?? cf Associtivity
#+END_SRC
...
#
For ~Sum~, apply a function in ~f₁ (a → b) + f₂ (a → b)~ to the same tagged element
in ~f₁ a + f₂ a~ provided we have a /coherent/ translation ~η : f₁ α → f₂ α~. E.g.,
~Maybe α ≅ Identity α + Const ()~ with ~η = const ()~.
#+BEGIN_SRC haskell
ef <$> ea = ((λ f → (f +++ (ap f . η)) ea) ||| (λ g → (ap(g . η) +++ g) ea)) ef
ap = (<*>)
#+END_SRC
# Axioms of monoidal natural transformations
# Axiom 0:  fmap f . η = η . fmap f    -- naturality, free by parametericity
# Axiom 1:  η . pure = pure            -- preserves pure
# Axiom 3:  η (f <$> x) = η f <$> η x  -- homomorphism
...
# Reader let's us simulate “global, read-only, data”.
